module AdwordsSimpleApi
  class FeedItem < Base
    VALUE_ATTRIBUTE_BY_TYPE = {
      ...
      'INT64_LIST' => { :value_key => :integer_values, :to_value => lambda{|a| AdwordsSimpleApi.wrap(a).map(:to_i) } },
      'FLOAT_LIST' => { :value_key => :double_values, :to_value => lambda{|a| AdwordsSimpleApi.wrap(a).map(&:to_f) } },
      'STRING_LIST' => { :value_key => :string_values, :to_value => lambda{|a| AdwordsSimpleApi.wrap(a).map(&:to_s) } },
      'BOOLEAN_LIST' => { :value_key => :boolean_values, :to_value => lambda{|a| AdwordsSimpleApi.wrap(a).map{|x| !!x } } },
      'URL_LIST' => { :value_key => :string_values, :to_value => lambda{|a| AdwordsSimpleApi.wrap(a).map(&:to_s) } },
      'DATE_TIME_LIST' => { :value_key => :string_values, :to_value => lambda{|a| AdwordsSimpleApi.wrap(a).map(&:to_s) } },
